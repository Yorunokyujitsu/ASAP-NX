...
putenv('GDFONTPATH=' . realpath(__DIR__ . '/../../src/fonts'));
...
if($offline == 0) {
    ...
    if(isset($_POST["font"])) {
        $font_name = basename($_POST["font"]);
        $font_file = __DIR__ . "/../../src/fonts/" . $font_name;
    } else {
        echo("Mising Font\n");
        exit(0);
    }
...
if($builtin_sym) {
    ...
    $font_file = __DIR__ . "/../../src/fonts/fontawesome.ttf";
 ...
 * $font_name $h_px px Font in $unicode_start_str ($unicode_start_letter) .. $unicode_last_str ($unicode_last_letter)  range with all bpp";
    ...
    $c_src = <<<EOT
    /*
     * Copyright (c) 2019-2026 CTCaer, Custom font: Asa, Yorunokyujitsu
     *
...
{\n";
$c_glyph_bitmap .= "#if USE_".strtoupper($output_name)." == 8\n";
$c_glyph_dsc = "
/*Store the glyph descriptions*/
static const lv_font_glyph_dsc_t $output_name" . "_glyph_dsc[] =
{\n";
$c_glyph_dsc   .= "#if USE_".strtoupper($output_name)." == 8\n";
...
$c_font_dsc .= "\n#if USE_" . strtoupper($output_name) . " == 1
    ...
    $c_font_dsc .= "
#if USE_".strtoupper($output_name)." == 8\n    .bpp = $bpp,\t\t\t\t/*Bit per pixel*/\n#endif\n";
}
...
function convert_all_letters()
{
    ...
    if(count($utf8_array) == 0) {
        for($unicode_act = $unicode_start; $unicode_act <= $unicode_last; $unicode_act++) {
            ...
            $h_pt_i = (int)$h_pt;
            $y_pt_i = (int)($h_pt + $base_line_ofs);
            $co = imagettftext($im, $h_pt_i, 0, 0, $y_pt_i, $white, $font_file, $letter);    //Size and Y in pt NOT px
            $w_px = $co[1] - $co[0];
        ...
        for($i = 0; $i < count($unicode_array); $i++) {
            ...
            $co = imagettftext($im, $h_pt_i, 0, 0, $y_pt_i, $white, $font_file, $utf8_array[$i]);    //Size and Y in pt NOT px
...
function height_corr()
{
    ...
    $h_pt_i = (int)$h_pt;
    $im = imagecreatetruecolor($h_pt_i, $h_pt_i);  /*The size is not important*/
    $white = imagecolorallocate($im, 255, 255, 255);
    $black = imagecolorallocate($im, 0, 0, 0);
    $max_h = $h_px;
    ...
    while($h_pt > 0) {
        $h_pt_i = (int)$h_pt;
        $co = imagettftext($im, $h_pt_i, 0, 0, $h_pt_i, $white, $font_file, $test_text);
